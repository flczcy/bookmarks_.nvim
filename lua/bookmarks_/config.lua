...
M.schema = {
  ...
  save_file = {
    ...
    default = vim.fs.normalize(vim.fn.stdpath("state") .. "/bookmarks_"),
  },
  sign_priority = {
    type = "number",
    default = 6,
...
local warn = function(s, ...)
  vim.notify(s:format(...), vim.log.levels.WARN, { title = 'bookmarks_' })
end
...
local function validate_config(config)
  for k, v in pairs(config) do
    ...
    if kschema == nil then
      warn("bookmarks_: Ignoring invalid configuration field '%s'", k)
